...
class SubtractionViewController: UIViewController {
    ...
    var subtractionArray: [ClassifyModel]?
    ...
    */
    ...
    @IBAction func onNextButtonClick(_ sender: Any) {
        if ((self.subtractionArray?.count ?? 0) - 1) > self.currentArrayIndex {
            self.currentArrayIndex = self.currentArrayIndex + 1
        }
        self.updateUI()
    }
    ...
    @IBAction func onFirstButtonClick(_ sender: Any) {
        self.checkAnswer(answer: self.subtractionArray?[self.currentArrayIndex].options[0].answer ?? "")
    }
    
    @IBAction func onSecondButtonClick(_ sender: Any) {
        self.checkAnswer(answer: self.subtractionArray?[self.currentArrayIndex].options[1].answer ?? "")
    }
    
    @IBAction func onThirdButtonClick(_ sender: Any) {
        self.checkAnswer(answer: self.subtractionArray?[self.currentArrayIndex].options[2].answer ?? "")
    }
    ...
    private func updateUI() {
        self.headerLabel.text = self.subtractionArray?[self.currentArrayIndex].header
        if let additionArray = self.subtractionArray {
            ...
            self.questionLabel.text = self.subtractionArray?[self.currentArrayIndex].title
            
            let firstButtonTitle = self.subtractionArray?[self.currentArrayIndex].options[0].title
            self.firstButton.setTitle(firstButtonTitle, for: .normal)
            
            let secondButtonTitle = self.subtractionArray?[self.currentArrayIndex].options[1].title
            self.secondButton.setTitle(secondButtonTitle, for: .normal)
            
            let thirdButtonTitle = self.subtractionArray?[self.currentArrayIndex].options[2].title
